...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, mask  = batchify_with_label(instance, instance_text, data.HP_gpu, False, data.sentence_classification)
...
def batchify_with_label(input_batch_list, input_text_batch_list, gpu, if_train=True, sentence_classification=False):
    if sentence_classification:
        ...
        raise RuntimeError('only support for sequence labeling')
    else:
...
def batchify_sequence_labeling_with_label(input_batch_list, input_text_batch_list, gpu, if_train=True):
    ...
    words = [sent[0] for sent in input_text_batch_list]
    char_seq_tensor = batch_to_ids(words)
    ...
    word_seq_recover = torch.arange(batch_size, dtype=torch.int64)
    ...
    return None, None, word_seq_lengths, word_seq_recover, char_seq_tensor, None, None, label_seq_tensor, mask


from elmo.elmo_seqlabel import Elmo_SeqLabel
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if not instance:
                continue
            batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, mask  = batchify_with_label(instance, instance_text, data.HP_gpu, True, data.sentence_classification)
            instance_count += 1
